...
from utils import random_patching, horizontal_flip, vertical_flip
from functools import partial


#TODO
# Fix passing the root_dir arg
...
class VOCDataset():
    ...
    def __init__(self, root_dir, default_boxes,
                 new_size, num_examples=-1, augmentation='flip'):
        ...
        if augmentation == 'flip':
            self.augmentation = ['original'] + ['horizontal_flip', 'vertical_flip']   
        else:
    ...
    def generate(self, subset=None, aug_coefficent = 0.05):
        ...
        for index in range(len(indices)):
            ...
            elif augmentation_method == 'horizontal_flip':
                img, boxes, labels = horizontal_flip(img, boxes, labels)
            elif augmentation_method == 'vertical_flip':
            ...
            img = tf.image.random_contrast(img, 1.0 - aug_coefficent, 1.0 + aug_coefficent)
            img = tf.image.random_hue(img, aug_coefficent)
            img = tf.image.random_saturation(img, 1.0 - aug_coefficent, 1.0 + aug_coefficent)
